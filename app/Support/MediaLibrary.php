...
class MediaLibrary
{
        ...
        $images = collect($disk->allFiles())
            ...
            ->map(function (string $path) use ($disk, $metadata): array {
                ...
                return [
                    ...
                    'source_id' => $imageMetadata?->source_id,
                    'source_url' => $imageMetadata?->source_url,
                    'source_author_name' => $imageMetadata?->source_author_name,
                    'source_author_url' => $imageMetadata?->source_author_url,
    ...
    private static function searchHaystack(array $image): string
    {
        return collect([
            ...
            $image['source'] ?? null,
            $image['source_id'] ?? null,
            $image['source_author_name'] ?? null,
            $image['source_url'] ?? null,
            $image['source_author_url'] ?? null,
            $image['path'] ?? null,
